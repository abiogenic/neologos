...
def phone_rasing(word,qualifying_vowels=list(mid_vowels + close_mid_vowels + near_close_vowels + close_vowels)):										
...
# 	Phone Rasing					tet > tit						low or mid vowels raise to mid or high vowels
	for key in word.dict_stripped_diacritics.keys():
		if word.dict_stripped_diacritics[key] in qualifying_vowels:
			v = raise_vowel(word.dict_stripped_diacritics[key])
			word = change_sound(word,v,key)
	return(word)

def phone_lowering(word,qualifying_vowels=list(open_vowels + near_open_vowels + open_mid_vowels + mid_vowels)):
																	# vowels is a list of one or more vowels (including category lists from "sounds.py")
